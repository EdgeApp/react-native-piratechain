// Builds the SwiftPM dependency graph that the vendored PirateLightClientKit
// source links against — grpc-swift, SwiftNIO, SwiftProtobuf, SQLite.swift and
// their C shims — into ONE static library per platform packaged as an
// XCFramework, plus the Swift `.swiftmodule`s and C `module.modulemap`s the
// in-pod SDK source needs to `import` them.
//
...
//
// PROVIDER / CONSUMER (read this before touching the podspec):
// react-native-zcash links the SAME dependency graph and vendors it the same
// way. The Edge app ships BOTH pods. Two copies of the graph on one app link
// line collide under the app's `-ObjC` flag (it force-loads every Swift archive
// member → tens of thousands of duplicate symbols). So exactly ONE pod may put
// the graph on the link line. This package builds and ships everything needed
// to be the PROVIDER (the default — it works standalone), but the podspec lets
// the host app demote it to a CONSUMER (RN_PIRATECHAIN_VENDOR_DEPS=false) when
// another pod already provides an identical copy. In CONSUMER mode this pod
// still compiles its in-pod source against the harvested modules, but omits its
// own libPirateDeps.xcframework from the link; its grpc/NIO symbols resolve
// from the provider's archive. The invariant that makes that safe:
// scripts/depsPackage.resolved pins this graph to the EXACT versions
// react-native-zcash resolves, so the two archives are interchangeable.
//
// Output (all under ios/vendored/, gitignored, shipped in the npm tarball):
//   libPirateDeps.xcframework - merged static lib (device arm64; sim arm64+x86_64)
...
// exact Swift compiler that wrote it (library-evolution .swiftinterface is not
// an option: swift-nio rejects evolution builds by upstream policy — see
// apple/swift-nio#2470/#2897, closed "not planned"). swift-version.txt records
// the producing compiler so the podspec can fail fast with instructions when
// the consuming Xcode doesn't match.
...
const sdkClone = join(tmp, 'PirateLightClientKit')
const wrapper = join(tmp, 'deps-wrapper')
const vendored = join(root, 'ios/vendored')

// Checked-in dependency pins for the wrapper build. The Pirate SDK repo does
// not commit a Package.resolved (its Package.swift uses open ranges like
// grpc-swift `from: 1.14.0`), so this snapshot pins the graph to the SAME
// versions react-native-zcash's SDK resolves — the PROVIDER/CONSUMER invariant.
const DEPS_RESOLVED = join(__dirname, 'depsPackage.resolved')

// Targets that must NOT go into the deps binary:
//  - PirateLightClientKit: compiled in-pod from source, not vendored as a binary
//  - PirateDepsWrapper: our throwaway entry-point target
// (libpiratelc is a binary target — no objects to merge; it ships separately
// as ios/libpiratelc.xcframework. sqlite3 needs no exclusion: on Apple
// platforms SQLite.swift imports the system `sqlite3` clang module, so its
// sqlite3_* references resolve at app link time from whatever the host links.)
const EXCLUDED_TARGETS = ['PirateLightClientKit', 'PirateDepsWrapper']
...
// Arch baseline matches libpiratelc.xcframework: arm64 devices,
// arm64 + x86_64 simulators (so Intel Macs can still build).
...
export function buildVendoredDeps(): void {
  ...
  for (const platform of PLATFORMS) {
    ...
    // poisons the build with stale precompiled modules of the binary-target
    // header, surfacing as bogus cannot-find-FFI-symbol errors.
    rm(dd)
    loud(wrapper, [
      'xcodebuild',
      '-scheme',
      ...
      '-derivedDataPath',
      dd,
      // Honor the checked-in depsPackage.resolved; hard-fail on any version
      // drift instead of silently re-resolving:
  ...
  // step silently ignores). The podspec only links it in PROVIDER mode.
  console.log('  Creating libPirateDeps.xcframework...')
  const xcframework = join(vendored, 'libPirateDeps.xcframework')
  rm(xcframework)
  const libArgs: string[] = []
  for (const platform of PLATFORMS) {
    // CocoaPods requires a UNIFORM library basename across xcframework slices;
    // stage each platform's lib under the same name in its own directory. The
    // name also differs from react-native-zcash's libZcashDeps.a so that, if a
    // future host ever links both archives, the -l flags don't clash:
    const stage = join(tmp, `xcfw-${platform.dir}`)
    rm(stage)
    mkdirSync(stage, { recursive: true })
    cp(
      join(vendored, `libPirateDeps-${platform.dir}.a`),
      join(stage, 'libPirateDeps.a')
    )
  ...
  for (const platform of PLATFORMS) {
    rm(join(vendored, `libPirateDeps-${platform.dir}.a`))
  }
...
// SDK's dependency graph; we then harvest those compiled deps.
function writeWrapperPackage(): void {
  rm(wrapper)
  mkdirSync(join(wrapper, 'Sources/PirateDepsWrapper'), { recursive: true })
  writeFileSync(
    join(wrapper, 'Package.swift'),
    `// swift-tools-version:5.9
import PackageDescription
let package = Package(
  name: "PirateDepsWrapper",
  // Match the pod's floor (iOS 13) so the prebuilt deps import cleanly from
  // any host at or above it (Edge develop pins 15.6):
  platforms: [.iOS(.v13)],
  products: [.library(name: "PirateDepsWrapper", type: .static, targets: ["PirateDepsWrapper"])],
  dependencies: [.package(path: ${JSON.stringify(sdkClone)})],
  targets: [.target(name: "PirateDepsWrapper", dependencies: [
    .product(name: "PirateLightClientKit", package: "PirateLightClientKit")
  ])]
)
`
  )
  writeFileSync(
    join(wrapper, 'Sources/PirateDepsWrapper/Empty.swift'),
    '@_exported import PirateLightClientKit\n'
  )
  if (!existsSync(DEPS_RESOLVED)) {
    throw new Error(
      `Missing ${DEPS_RESOLVED} - the checked-in dependency pins. ` +
        'Regenerate by building the wrapper once without ' +
        '-disableAutomaticPackageResolution and committing the resulting ' +
        'Package.resolved (keeping the shared packages at the versions ' +
        'react-native-zcash resolves).'
    )
  }
  cp(DEPS_RESOLVED, join(wrapper, 'Package.resolved'))
}
...
function mergeDeps(dd: string, platform: Platform): void {
  ...
  for (const arch of platform.archs) {
    ...
    const archLib = join(tmp, `libPirateDeps-${platform.dir}-${arch}.a`)
    rm(archLib)
    loud(tmp, ['libtool', '-static', '-o', archLib, '-filelist', listFile])
    archLibs.push(archLib)
  }

  const out = join(vendored, `libPirateDeps-${platform.dir}.a`)
  rm(out)
